@php $rev = $missionControl['revenue'] ?? []; @endphp
<div class="space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <div class="glass-card p-6 md:col-span-2">
            ...
            <div class="flex flex-wrap items-end justify-between gap-4 mt-2">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-700">{{ $rev['target_formatted'] ?? '—' }}</p>
                </div>
                <div class="text-right">
                    <p class="text-xs text-gray-500">Achieved (invoiced)</p>
                    <p class="text-2xl sm:text-3xl font-black text-indigo-700">{{ $rev['achieved_formatted'] ?? '₹ 0' }}</p>
                </div>
            </div>
            @if($rev['progress_percent'] !== null)
            <div class="mt-4">
        ...
        <div class="glass-card p-6">
            ...
            <p class="text-4xl font-black text-emerald-600 mt-2">{{ $rev['collection_efficiency'] ?? 0 }}%</p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="glass-card p-5 kpi-emerald">
            ...
            <p class="text-2xl font-black text-emerald-700">{{ $rev['outstanding_formatted'] ?? $summary['outstanding_fees'] ?? '₹ 0' }}</p>
        ...
        <div class="glass-card p-5 kpi-violet">
            ...
            <p class="text-2xl font-black text-violet-700">₹ {{ number_format($rev['collected_mtd'] ?? 0, 0) }}</p>
        ...
        <div class="glass-card p-5 kpi-rose">
            ...
            <p class="text-2xl font-black text-rose-700">{{ $summary['overdue_collections'] ?? '₹ 0' }}</p>
