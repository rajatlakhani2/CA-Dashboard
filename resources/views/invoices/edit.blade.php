...
<div class="flex justify-between items-center w-full">
    ...
    <a href="{{ route('invoices.show', $invoice) }}" class="text-sm text-indigo-600 hover:text-indigo-900">View Invoice</a>
</div>
@endsection
        ...
        <div class="bg-bg-card shadow sm:rounded-lg border border-line">
            ...
            <div class="px-4 py-5 sm:p-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="invoice_number" id="invoice_number" value="{{ $invoice->invoice_number }}" required class="mt-1 block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>

                <!-- Dates -->
                <div class="col-span-1">
                    <label for="date" class="block text-sm font-medium text-text-secondary">Invoice Date</label>
                    <input type="date" name="date" id="date" value="{{ $invoice->date->format('Y-m-d') }}" required class="mt-1 block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>

                <div class="col-span-1">
                    <label for="due_date" class="block text-sm font-medium text-text-secondary">Due Date</label>
                    <input type="date" name="due_date" id="due_date" value="{{ $invoice->due_date->format('Y-m-d') }}" required class="mt-1 block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                    ...
                    <tbody id="items-container" class="divide-y divide-line">
                        ...
                        <tr>
                            <td class="py-2 pr-2">
                                ...
                                <input type="text" name="items[{{ $index }}][description]" value="{{ $item->description }}" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </td>
                            <td class="py-2 pr-2">
                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" step="0.01" min="0" oninput="calculateTotal()" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </td>
                            <td class="py-2 pr-2">
                                <input type="number" name="items[{{ $index }}][rate]" value="{{ $item->rate }}" step="0.01" min="0" oninput="calculateTotal()" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </td>
            ...
            <div class="px-4 py-5 sm:p-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ $invoice->notes }}</textarea>
                </div>

                <div class="flex flex-col items-end justify-start space-y-2">
                    <div class="flex justify-between w-48 text-sm">
                        <span class="text-text-secondary">Subtotal:</span>
                        <span class="font-medium text-text-main" id="subtotal-display">₹ {{ number_format($invoice->subtotal, 2) }}</span>
                    </div>
                    <div class="flex justify-between w-48 text-sm">
                    ...
                    <div class="flex justify-between w-48 text-base font-bold border-t border-line pt-2">
                        ...
                        <span class="text-indigo-600" id="total-display">₹ {{ number_format($invoice->total_amount, 2) }}</span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function addItem() {
        ...
        row.innerHTML = `
            <td class="py-2 pr-2">
                <input type="text" name="items[${itemIndex}][description]" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </td>
            <td class="py-2 pr-2">
                <input type="number" name="items[${itemIndex}][quantity]" value="1" step="0.01" min="0" oninput="calculateTotal()" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </td>
            <td class="py-2 pr-2">
                <input type="number" name="items[${itemIndex}][rate]" value="0" step="0.01" min="0" oninput="calculateTotal()" required class="block w-full rounded-md border-line bg-bg-body text-text-main shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </td>
            <td class="py-2 text-right text-sm text-text-main font-medium item-amount">
                ₹ 0.00
            </td>
