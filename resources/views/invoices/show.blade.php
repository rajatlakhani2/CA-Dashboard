...
<div class="flex justify-between items-center w-full print:hidden">
    ...
    <div class="flex flex-wrap gap-2 items-center" data-demo-tour="invoice-send-actions">
        @if($invoice->status !== 'Paid')
        <form action="{{ route('invoices.send-email', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('Send this invoice by email to the client?')">
            @csrf
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded shadow flex items-center font-semibold">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                Email invoice
            </button>
        </form>
        <form action="{{ route('invoices.whatsapp', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('Send WhatsApp payment reminder to the client?')">
            @csrf
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded shadow flex items-center font-semibold">
