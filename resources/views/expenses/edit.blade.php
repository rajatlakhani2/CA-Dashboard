...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden animate-enter">
        <div class="bg-slate-900 px-8 py-8 flex items-center justify-between">
            <div>
                <h3 class="text-xl font-bold text-white">Modify Outflow</h3>
                <p class="text-slate-400 text-xs mt-1 uppercase tracking-widest font-bold">Edit Expense Entry</p>
            </div>
        ...
        <form action="{{ route('expenses.update', $expense) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="col-span-1">
                    ...
                    <input type="date" name="expense_date" value="{{ old('expense_date', $expense->expense_date ? $expense->expense_date->format('Y-m-d') : '') }}" required class="w-full bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all py-3 px-4 font-bold text-gray-800">
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="amount" value="{{ old('amount', $expense->amount) }}" required placeholder="0.00" class="w-full bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all py-3 px-4 font-black text-lg text-gray-900 placeholder-gray-300">
                ...
                <div class="col-span-1">
                    ...
                    <select name="payment_mode" class="w-full bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all py-3 px-4 font-bold text-gray-800">
                        <option value="Bank Transfer" {{ old('payment_mode', $expense->payment_mode) == 'Bank Transfer' ? 'selected' : '' }}>Bank Transfer</option>
                        <option value="UPI" {{ old('payment_mode', $expense->payment_mode) == 'UPI' ? 'selected' : '' }}>UPI / GPay</option>
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="vendor" value="{{ old('vendor', $expense->vendor) }}" placeholder="e.g. Amazon Cloud, Office Landlord, DTDC" class="w-full bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all py-3 px-4 font-bold text-gray-800">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="description" value="{{ old('description', $expense->description) }}" required placeholder="Short description of this expense" class="w-full bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all py-3 px-4 font-bold text-gray-800">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <div class="mb-2 text-xs text-indigo-600 font-bold">
                        Current attachment: <a href="{{ asset('storage/' . $expense->receipt_path) }}" target="_blank" class="underline">View Receipt</a>
                    </div>
                    @endif
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-100 border-dashed rounded-3xl bg-gray-50 hover:bg-white hover:border-indigo-300 transition-all cursor-pointer group relative">
                        ...
                        <div class="space-y-2 text-center">
                            ...
                            <div class="text-sm text-gray-600">
                                <span class="font-bold text-indigo-600">Upload a new file</span> or drag and drop
                            </div>
                            <p class="text-xs text-gray-500 font-bold uppercase tracking-tighter">PNG, JPG, PDF up to 2MB (leave blank to keep current)</p>
                        </div>
                    </div>
                </div>
            </div>
